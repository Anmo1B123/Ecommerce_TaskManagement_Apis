...
export const uploadOnCloudinary= async function(localfilepath, public_id=undefined){
    try {                                                                                           //public_id as an argument will come- would be a string and then that name will be shown on cloudinary
        if (localfilepath) {const response = await cloudinary.uploader.upload(localfilepath, {public_id:public_id,
            resource_type: "auto"});
